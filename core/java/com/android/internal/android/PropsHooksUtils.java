...
public class PropsHooksUtils {
    ...
    // System Properties Obfuscated
    private static final String OPTIMIZE_FRAMEWORK_GMS = "persist.sys.framework.opt_a";
    private static final String OPTIMIZE_FRAMEWORK_GPHOTOS = "persist.sys.framework.opt_b";
    public static final String OPTIMIZE_FRAMEWORK_GAMES = "persist.sys.framework.opt_c";

    private static final String PROP_PREFIX_KEYS = "persist.sys.fwk_cfg_k";
    private static final String PROP_PREFIX_VAL = "persist.sys.fwk_cfg_v_";
    ...
    private static final ArrayMap<String, Map<String, Object>> pubgPropsMap = new ArrayMap<>();
    ...
    static {
        ...
        // Profil Pixel XL untuk Google Photos
        ...
        addToMap(Set.of("com.epicgames.fortnite", "com.epicgames.portal", "com.tencent.lolm"), propsOP9P);
        ...
        for (String pkg : pubgPackages) {
            pubgPropsMap.put(pkg, propsS24Ultra);
        }
    ...
    public static void setProps(Context context) {
        ...
        // Logic Spoofing Game
        if (shouldSpoofGames()) {
            Map<String, Object> props = pubgPropsMap.containsKey(packageName)
                    ? pubgPropsMap.get(packageName)
                    : packagePropsMap.get(packageName);
        ...
        if (packageName.equals("com.google.android.settings.intelligence")) {
            setPropValue(packageName, "FINGERPRINT", "build." + System.currentTimeMillis());
        }

        // Logic Spoofing GMS / Play Store (Play Integrity)
        if (shouldSpoofGMS(packageName, processName)) {
            spoofBuildGms(packageName);
        }
    }
    ...
    private static boolean shouldSpoofGames() {
        return SystemProperties.getBoolean(OPTIMIZE_FRAMEWORK_GAMES, false);
    }

    private static boolean shouldSpoofPhotos() {
        return sIsPhotos && SystemProperties.getBoolean(OPTIMIZE_FRAMEWORK_GPHOTOS, true);
    }

    private static boolean shouldSpoofGMS(String packageName, String processName) {
        final boolean isGmsUnstable = packageName.equals("com.google.android.gms") 
                && processName.toLowerCase(Locale.US).endsWith(":unstable");
        final boolean isFinsky = packageName.equals(FINSKY);
        return (isGmsUnstable || isFinsky) && SystemProperties.getBoolean(OPTIMIZE_FRAMEWORK_GMS, true);
    }

    private static void spoofBuildGms(String pkg) {
        try {
            String keys = SystemProperties.get(PROP_PREFIX_KEYS, "");
            
            // Fallback jika tidak ada pif.json
            if (TextUtils.isEmpty(keys)) {
                setPropValue(pkg, "DEVICE_INITIAL_SDK_INT", 25);
                setPropValue(pkg, "MANUFACTURER", "Google");
                setPropValue(pkg, "BRAND", "google");
                setPropValue(pkg, "PRODUCT", "marlin");
                setPropValue(pkg, "DEVICE", "marlin");
                setPropValue(pkg, "MODEL", "Pixel XL");
                setPropValue(pkg, "FINGERPRINT", "google/marlin/marlin:7.1.2/NJH47F/4146041:user/release-keys");
                setPropValue(pkg, "SECURITY_PATCH", "2017-10-05");
                return;
            }

            ArrayMap<String, Object> propsToApply = new ArrayMap<>();
            for (String key : keys.split(",")) {
                String value = SystemProperties.get(PROP_PREFIX_VAL + key, null);
            ...
            if (!propsToApply.containsKey("DEVICE_INITIAL_SDK_INT")) {
                setPropValue(pkg, "DEVICE_INITIAL_SDK_INT", 25);
            }
